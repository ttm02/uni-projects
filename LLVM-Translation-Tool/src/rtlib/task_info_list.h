#ifndef TASKINFO_LIST_H_
#define TASKINFO_LIST_H_
...
// same as mpi_list.h but Typed with task_info, as copy template class into module is not that
// easy

// do not suggest to inline any of this calls
...
typedef struct task_info_struct
{
    ...
    int *LOCAL_shared_param_list;
} task_info;

class task_info_list
{
  ...
  private:
    ...
    MPI_Aint alloc_elem(task_info *value)
    {
        MPI_Aint disp;
        ...
        memcpy(&elem_ptr->value, value, sizeof(task_info));
        // nil ptr
  ...
  public:
    ...
    task_info_list(int basis_rank = 0)
    {
        ...
        MPI_Bcast(&head_disp, 1, MPI_AINT, basis_rank, MPI_COMM_WORLD);
        MPI_Bcast(&tail_disp, 1, MPI_AINT, basis_rank, MPI_COMM_WORLD);
    ...
    void push_back(task_info *value)
    {
        int rank;
        MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    ...
    void enqueue(task_info *value) { this->push_back(value); }

    // returns value of elem or given value if list is empty
    void peek_last(task_info *result)
    {
        ...
        if (tail_ptr.rank == -1)
        {
            memset(result, 0, sizeof(task_info));
            return;
        }
        // fetch element
        llist_elem_t elem;
        ...
        memcpy(result, &elem.value, sizeof(task_info));
    }

    void peek(task_info *result)
    {
    ...
    // other name of same function if it should be used as a queue
    void dequeue(task_info *result) { this->pop(result); }

    // singular call: insert element at the beginning
    void push(task_info *value)
    {
        int rank;
        MPI_Comm_rank(MPI_COMM_WORLD, &rank);
        llist_ptr_t new_elem_ptr;
