// SAME AS IN LLVM SOURCE (Copy from sources/llvm/lib/Transforms/Scalar/SCCP.cpp)
// but removes static qualifier from runSCCP Function and removed anonymous namespace
...
class SCCPSolver : public InstVisitor<SCCPSolver>
{
  ...
  public:
    ...
    SCCPSolver(const DataLayout &DL, const TargetLibraryInfo *tli) : DL(DL), TLI(tli) {}
    ...
    void AddArgumentTrackedFunction(Function *F) { TrackingIncomingArguments.insert(F); }

    /// Returns true if the given function is in the solver's set of
    /// argument-tracked functions.
    bool isArgumentTrackedFunction(Function *F) { return TrackingIncomingArguments.count(F); }
    ...
    bool isBlockExecutable(BasicBlock *BB) const { return BBExecutable.count(BB); }
    ...
    {
        assert(!V->getType()->isStructTy() && "Should use getStructLatticeValueFor");
        DenseMap<Value *, LatticeVal>::const_iterator I = ValueState.find(V);
        assert(I != ValueState.end() && "V not found in ValueState nor Paramstate map!");
        return I->second;
    }

    /// getTrackedRetVals - Get the inferred return value map.
    const DenseMap<Function *, LatticeVal> &getTrackedRetVals() { return TrackedRetVals; }
    ...
    const SmallPtrSet<Function *, 16> getMRVFunctionsTracked() { return MRVFunctionsTracked; }

    /// getMustTailCallees - Get the set of functions which are called
    /// from non-removable musttail call sites.
    const SmallPtrSet<Function *, 16> getMustTailCallees() { return MustTailCallees; }
  ...
  private:
    ...
    bool markOverdefined(LatticeVal &IV, Value *V)
    {
        ...
        LLVM_DEBUG(dbgs() << "markOverdefined: "; if (auto *F = dyn_cast<Function>(V)) dbgs()
                                                  << "Function '" << F->getName() << "'\n";
                   else dbgs() << *V << '\n');
    ...
    {
        assert(!V->getType()->isStructTy() && "non-structs should use markConstant");
    ...
    ValueLatticeElement &getParamState(Value *V)
    {
        ...
        std::pair<DenseMap<Value *, ValueLatticeElement>::iterator, bool> PI =
            ParamState.insert(std::make_pair(V, ValueLatticeElement()));
    ...
    LatticeVal &getStructValueState(Value *V, unsigned i)
    {
        ...
        std::pair<DenseMap<std::pair<Value *, unsigned>, LatticeVal>::iterator, bool> I =
            StructValueState.insert(std::make_pair(std::make_pair(V, i), LatticeVal()));
        LatticeVal &LV = I.first->second;
...
// getFeasibleSuccessors - Return a vector of booleans to indicate which
// successors are reachable from a given terminator instruction.
void SCCPSolver::getFeasibleSuccessors(Instruction &TI, SmallVectorImpl<bool> &Succs)
{
...
void SCCPSolver::visitCastInst(CastInst &I)
{
    ...
    else if (OpSt.isConstant())
    {
        ...
        Constant *C =
            ConstantFoldCastOperand(I.getOpcode(), OpSt.getConstant(), I.getType(), DL);
        if (isa<UndefValue>(C))
            return;
...
void SCCPSolver::visitSelectInst(SelectInst &I)
{
    ...
    // select ?, C, C -> C.
    if (TVal.isConstant() && FVal.isConstant() && TVal.getConstant() == FVal.getConstant())
...
void SCCPSolver::visitBinaryOperator(Instruction &I)
{
    ...
    {
        Constant *C =
            ConstantExpr::get(I.getOpcode(), V1State.getConstant(), V2State.getConstant());
        // X op Y -> undef.
        if (isa<UndefValue>(C))
            return;
...
void SCCPSolver::visitCmpInst(CmpInst &I)
{
    ...
    ValueLatticeElement V1State =
        (V1Param != ParamState.end()) ? V1Param->second : getValueState(Op1).toValueLattice();

    auto V2Param = ParamState.find(Op2);
    ValueLatticeElement V2State =
        V2Param != ParamState.end() ? V2Param->second : getValueState(Op2).toValueLattice();
    ...
    if (!V1State.isOverdefined() && !V2State.isOverdefined() && !ValueState[&I].isConstant())
        return;
...
void SCCPSolver::visitGetElementPtrInst(GetElementPtrInst &I)
{
    ...
    Constant *C = ConstantExpr::getGetElementPtr(I.getSourceElementType(), Ptr, Indices);
    if (isa<UndefValue>(C))
        return;
    markConstant(&I, C);
}
...
void SCCPSolver::visitLoadInst(LoadInst &I)
{
    ...
    if (auto *GV = dyn_cast<GlobalVariable>(Ptr))
    {
        if (!TrackedGlobals.empty())
        {
            ...
            DenseMap<GlobalVariable *, LatticeVal>::iterator It = TrackedGlobals.find(GV);
...
void SCCPSolver::visitCallSite(CallSite CS)
{
    ...
    CallOverdefined:
        ...
        {
            SmallVector<Constant *, 8> Operands;
            for (CallSite::arg_iterator AI = CS.arg_begin(), E = CS.arg_end(); AI != E; ++AI)
            {
...
void SCCPSolver::Solve()
{
    ...
    while (!BBWorkList.empty() || !InstWorkList.empty() || !OverdefinedInstWorkList.empty())
    {
...
static bool tryToReplaceWithConstant(SCCPSolver &Solver, Value *V)
{
    ...
    if (V->getType()->isStructTy())
    {
        ...
        if (llvm::any_of(IVs, [](const LatticeVal &LV) { return LV.isOverdefined(); }))
            return false;
        ...
        {
            LatticeVal V = IVs[i];
            ConstVals.push_back(V.isConstant() ? V.getConstant()
...
bool runSCCP(Function &F, const DataLayout &DL, const TargetLibraryInfo *TLI)
{
...
INITIALIZE_PASS_BEGIN(SCCPLegacyPass, "sccp", "Sparse Conditional Constant Propagation", false,
                      false)
INITIALIZE_PASS_DEPENDENCY(TargetLibraryInfoWrapperPass)
INITIALIZE_PASS_END(SCCPLegacyPass, "sccp", "Sparse Conditional Constant Propagation", false,
                    false)

// createSCCPPass - This is the public interface to this file.
FunctionPass *llvm::createSCCPPass() { return new SCCPLegacyPass(); }

static void findReturnsToZap(Function &F, SmallVector<ReturnInst *, 8> &ReturnsToZap,
                             SCCPSolver &Solver)
{
...
static void forceIndeterminateEdge(Instruction *I, SCCPSolver &Solver)
{
    ...
    if (C)
    {
        assert(Solver.isEdgeFeasible(I->getParent(), Dest) && "Didn't find feasible edge?");
        (void)Dest;
...
bool llvm::runIPSCCP(Module &M, const DataLayout &DL, const TargetLibraryInfo *TLI,
                     function_ref<std::unique_ptr<PredicateInfo>(Function &)> getPredicateInfo)
{
    ...
    for (Function &F : M)
    {
        ...
        for (BasicBlock *BB : BlocksToErase)
            NumInstRemoved += changeToUnreachable(BB->getFirstNonPHI(), /*UseLLVMTrap=*/false);
        if (!Solver.isBlockExecutable(&F.front()))
        ...
        for (unsigned i = 0, e = BlocksToErase.size(); i != e; ++i)
        {
            ...
            for (Value::user_iterator UI = DeadBB->user_begin(), UE = DeadBB->user_end();
                 UI != UE;)
            {
    ...
    {
        assert(F->getReturnType()->isStructTy() && "The return type should be a struct");
    ...
    for (DenseMap<GlobalVariable *, LatticeVal>::const_iterator I = TG.begin(), E = TG.end();
         I != E; ++I)
    {
